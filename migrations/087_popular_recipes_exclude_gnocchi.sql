-- Migration 087: popular_saved_recipes() — keep the rosemary gnocchi reel off the shelf
--
-- After 086 the leaderboard showed healthygirlkitchen's rosemary gnocchi reel
-- (Instagram shortcode DGioQ5qOWij) five times: it is the recipe the default
-- seed was built from, so besides the (already excluded) default_seed rows it
-- also exists as real user imports under /p/, /reel/?igsh=, /reel/?igsi= URL
-- forms and two rows with no source_url whose titles differ only by a caption
-- suffix. Normalising the dedupe key would merge most of those; the product
-- call on 2026-09-08 was simpler — do not count this recipe at all.
--
-- Same signature, same body as 086, plus a two-line blocklist in the WHERE.
-- Both tests are needed because two of the variants carry no source_url.
-- Extend the blocklist there if another recipe ever needs the same treatment.
--
-- To REVERT (count it again): re-run 086 in full.
--
...
-- Verify
-- ---------------------------------------------------------------------------
-- 1. No row from that reel. Other gnocchi recipes (the lemon one, the pesto
--    tray bake, the chicken & bacon one) are unaffected and may still appear.
--   SELECT title, source_author, saver_count FROM public.popular_saved_recipes(20);
--   -- and specifically, this must return 0:
