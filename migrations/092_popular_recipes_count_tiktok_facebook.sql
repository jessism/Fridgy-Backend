-- Migration 092: popular_saved_recipes() — count TikTok and Facebook saves
--
-- 085-087 only ranked source_type instagram/web/popular, so a recipe saved from
-- TikTok or Facebook never counted toward Popular Now, however many people
-- saved it. That was an oversight, not a decision: PUBLIC_SOURCES
-- (services/recipeShareService.js), which the adopt route enforces, already
-- lists facebook and tiktok, so those cards could always be adopted. The
-- ranking had simply copied an older list.
--
-- Same signature, same body as 087; only the source_type allowlist changes.
--
-- To REVERT (instagram/web only again): re-run 087 in full.
--
...
WITH eligible AS (
  ...
  WHERE
    -- Only content from a public source may be shown to strangers. Must match
    -- PUBLIC_SOURCES in services/recipeShareService.js, which the adopt route
    -- enforces; manual/scanned/voice rows are the owner's own content and
    -- nobody consented to sharing them.
        r.source_type IN ('instagram', 'facebook', 'tiktok', 'web', 'popular')
    ...
    -- Blocklist. healthygirlkitchen's rosemary gnocchi reel (DGioQ5qOWij) is
    -- the recipe the default seed was built from, so it also exists as real
    -- user imports under several URL forms and two untitled variants. Product
    -- call 2026-09-08: keep it off the shelf entirely rather than merge them.
    -- Both tests, because two of the variants have no source_url at all.
    AND COALESCE(r.source_url, '') NOT ILIKE '%DGioQ5qOWij%'
    AND COALESCE(r.title, '')      NOT ILIKE '%rosemary gnocchi%'
...
COMMENT ON FUNCTION public.popular_saved_recipes(integer) IS
  'Home "Popular Now" leaderboard: public-source (instagram/facebook/tiktok/web/popular) recipes ranked by COUNT(DISTINCT user_id) over the source_url/title|author dedupe key. Excludes default_seed rows and the rosemary gnocchi reel DGioQ5qOWij (087 blocklist); test/internal accounts DO count (086). Returns no user_id, user_notes or rating. Backed by GET /api/recipes/popular.';

-- CREATE OR REPLACE keeps the grants from 085/086; restated so this file is
...
-- Verify
-- ---------------------------------------------------------------------------
-- 1. TikTok/Facebook rows can now appear (on 2026-09-22 one did: a TikTok
--    "Tomato Soup & Grilled Cheese" at 1 saver, 6th place):
--   SELECT title, source_type, saver_count FROM public.popular_saved_recipes(12);
--
-- 2. The 087 blocklist still holds; this must return 0:
--   SELECT COUNT(*) FROM public.popular_saved_recipes(50)
--   WHERE title ILIKE '%rosemary gnocchi%' OR COALESCE(source_url,'') ILIKE '%DGioQ5qOWij%';
--
-- 3. Grants unchanged: expect {postgres=X/postgres,service_role=X/postgres}.
--   SELECT proname, proacl FROM pg_proc WHERE proname = 'popular_saved_recipes';
--
-- 4. anon must still be refused (the 42501 error IS the pass):
