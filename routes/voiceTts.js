...
router.post('/speak', authMiddleware.authenticateToken, voiceTtsSpeakLimiter, async (req, res) => {
  ...
  if (!runpodTts.isKnownVoice(voiceId)) {
  ...
  try {
    ...
    res.json({ success: true, audioBase64: wav.toString('base64'), sampleRate: 24000 });
  } catch (err) {
...
router.get('/voices', authMiddleware.authenticateToken, (req, res) => {
  res.json({ success: true, voices: runpodTts.listVoices() });
});

router.get('/preview/:voiceId', authMiddleware.authenticateToken, (req, res) => {
